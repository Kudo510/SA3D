...
def _compute_bbox_by_cam_frustrm_bounded(cfg, HW, Ks, poses, i_train, near, far):
    # Initialize the minimum and maximum 3D coordinates with extreme values
    xyz_min = torch.Tensor([np.inf, np.inf, np.inf])
    xyz_max = -xyz_min
    
    # Loop through the specified training indices
    for (H, W), K, c2w in zip(HW[i_train], Ks[i_train], poses[i_train]):
        # Get the rays for the current view using the provided configuration and camera parameters
        rays_o, rays_d, viewdirs = dvgo.get_rays_of_a_view(
            H=H, W=W, K=K, c2w=c2w,
        ...
        # Calculate near and far points for the rays
        if cfg.data.ndc:
            # If normalized device coordinates (NDC) are used, use rays_d for calculations
            pts_nf = torch.stack([rays_o + rays_d * near, rays_o + rays_d * far])
        else:
            # If not using NDC, use view directions for calculations
            pts_nf = torch.stack([rays_o + viewdirs * near, rays_o + viewdirs * far])
        
        # Update the bounding box by taking the minimum and maximum of the current points
        xyz_min = torch.minimum(xyz_min, pts_nf.amin((0, 1, 2)))
        xyz_max = torch.maximum(xyz_max, pts_nf.amax((0, 1, 2)))
    
    # Return the computed minimum and maximum 3D coordinates
